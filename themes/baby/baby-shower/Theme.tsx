...
export default function BabyShowerTheme({
  ...
  usePetals(floatsRef, { color: "rgba(143,205,232,.45)", count: 14, shape: "round" });
  useTilt(cardRef);
  ...
  return (
    <div className={styles.root}>
      ...
      <div className={`${styles.gate} ${opened ? styles.out : ""}`} onClick={() => setOpened(true)}>
        ...
        <div className={styles.gateName}>{babyName}</div>
        <div className={styles.gateTag}>{content.invitationLine || "Baby Shower"}</div>
        ...
        <div className={styles.cardWrap} ref={cardRef}>
          <div className={styles.card}>
            ...
            <p className={`${styles.eyebrow} ${styles.rev}`} style={{ animationDelay: ".05s" }}>
              {content.invitationLine || "Baby Shower"}
            </p>
            ...
            <div className={`${styles.dateBlock} ${styles.rev}`} style={{ animationDelay: ".38s" }}>
            ...
            <p className={`${styles.closing} ${styles.rev}`} style={{ animationDelay: ".58s" }}>
              {content.closing}
            </p>
            ...
            <div className={`${styles.cta} ${styles.rev}`} style={{ animationDelay: ".65s" }}>
              ...
              {showRsvp && invitationId && (
                <button className={`${styles.btn} ${styles.btnPrimary}`}
                  onClick={() => document.getElementById("bs-rsvp")?.scrollIntoView({ behavior: "smooth" })}>
            ...
            {showRsvp && invitationId && (
              <section id="bs-rsvp" className={`${styles.rsvp} ${styles.rev}`} style={{ animationDelay: ".75s" }}>
                ...
                {rsvpSent || alreadyResponded ? (
                  <p className={styles.rsvpDone}>
                    {alreadyResponded && !rsvpSent ? "Vous avez déjà répondu — merci !"
                      : <>Merci <strong>{rsvpName}</strong> — à bientôt ! 🎀</>}
                  </p>
                ) : (
                  <form className={styles.rsvpForm} onSubmit={submitRsvp}>
                    ...
                    <div className={styles.toggle}>
                      {(["attending","declined"] as const).map((v, i) => (
                        <button key={v} type="button"
                          ...
                          onClick={() => setRsvpStatus(v)}>{i===0?"Je serai là 🎉":"Je ne pourrai pas"}</button>
                      ))}
                    </div>
                    ...
                    <textarea className={styles.inp} placeholder="Un petit mot (optionnel)"
