...
export default async function ManagePage({ params }: Props) {
  ...
  return (
    <div className="invytek-page" style={{ minHeight: "100dvh", paddingBottom: "4rem" }}>
      <Nav />
      <div className="wrap" style={{ paddingTop: 120, maxWidth: 860 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 12, marginBottom: "2.5rem" }}>
          {[
            { n: attending, label: "Confirment leur présence", color: "var(--gold)" },
            { n: declined,  label: "Ne peuvent pas venir",    color: "var(--text-faint)" },
            { n: pending,   label: "En attente de réponse",   color: "var(--text-soft)" },
          ].map(s => (
            ...
            {invitation.guests.length === 0 ? (
              ...
              <div style={{ maxHeight: 460, overflowY: "auto" }}>
                {invitation.guests.map(g => (
                  <div key={g.id} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "0.9rem 1.4rem", borderBottom: "1px solid var(--hair)", gap: 8 }}>
